...
public class FancyCoverFlowItemWrapper extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (this.wrappedViewBitmap == null
			...
			this.wrappedViewDrawingCanvas.setDrawFilter(filter);
		}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		canvas.drawBitmap(this.wrappedViewBitmap,
				(this.getWidth() - childView.getWidth()) / 2 + 1, 0, paint);
	}
		...
		if (child != null && child.getVisibility() != GONE) {
			...
			this.originalScaledownFactor = this.isReflectionEnabled ? (originalChildHeight
					* (1 - this.imageReflectionRatio) - reflectionGap)
					/ originalChildHeight
					: 1.0f;
			final int childHeight = (int) (this.originalScaledownFactor * originalChildHeight) + 1;
			final int childWidth = (int) (this.originalScaledownFactor * originalChildWidth) + 1;
	...
	private void createReflectedImages() {
		...
		final int scaledDownHeight = (int) (height * originalScaledownFactor) + 1;
		...
		this.wrappedViewDrawingCanvas.drawBitmap(invertedBitmap, 0,
				scaledDownHeight + reflectionGap, defaultPaint);
